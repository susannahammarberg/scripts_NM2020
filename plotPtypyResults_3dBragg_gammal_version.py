...
with h5py.File(inputFile, 'r') as hf:
    ...
    p_id = str(list(hf['content/pars/scans'].keys())[0])
    ...
    #energy = np.array(hf['content/probe/%s/_energy' % scanid])
    energy = np.array(hf['content/pars/scans/%s/data/energy' % p_id])
...
print(" pixel size %.1f x %.1f x %.1f nm, " %( tuple(psize*1E9) ))
...
extent = 1e6 * np.array([origin[1], origin[1]+(obj.shape[1]-1)*psize[1],origin[2], origin[2]+(obj.shape[2]-1)*psize[2]])
...
field3d = nmutils.utils.propagateNearfield(probe[int(probe.shape[0]/2)], psize, -dist, energy)
...
def spot_subplot(gridcell, data, shareax=None, title=''):
    ...
    return ax
a = spot_subplot(outer_grid[0,0], probe[int(probe.shape[0]/2)], title='Sample plane')
a.set_ylabel('$\mu$m')
...
# amplitude
mag = np.abs(obj[int(obj.shape[0]/2)])
...
# phase
img = ax[1].imshow(np.angle(obj[int(obj.shape[0]/2)]), vmin=-np.pi, vmax=np.pi, cmap='hsv', extent=extent, interpolation='none')
